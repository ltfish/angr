...
class ArrayIndexNaming(SemanticNamingBase):
    ...
    def _analyze_address_expr(self, addr_expr) -> None:
        """
    ...
    def _check_add_for_index(self, op0, op1) -> None:
        ...
        # Heuristic: if one operand is a constant (base address) and
        # the other is a variable, the variable might be an index
    ...
    def _assign_index_names(self) -> None:
        ...
        for var, _count in sorted_vars:
            # Check if variable already has a meaningful name
            if var.name and not var.name.startswith("v") and not var.name.startswith("var_"):
                continue

            # Also check the unified variable
