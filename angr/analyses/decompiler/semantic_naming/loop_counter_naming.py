...
class LoopCounterNaming(SemanticNamingBase):
    ...
    def _compute_nesting_depths(self) -> None:
        ...
        for head in sorted_heads:
            loop_nodes = self._loop_info[head]["nodes"]
            depth = 0
        ...
        for node in loop_nodes:
            ...
            for stmt in node.statements:
                ...
                if isinstance(src, BinaryOp) and src.op in ("Add", "Sub"):
                    ...
                    if op0_var == dst_var and isinstance(op1, Const):
                        increment_vars.add(dst_var)
                    elif op1_var == dst_var and isinstance(op0, Const):
                        increment_vars.add(dst_var)
    ...
    def _assign_counter_names(self) -> None:
        ...
        sorted_loops = sorted(
            self._loop_heads,
            key=lambda h: (self._loop_info[h]["nesting_depth"], h.addr)
        )
