...
class PointerNaming(SemanticNamingBase):
    ...
    def _check_address_expr(self, addr_expr, access_type: str) -> None:
    ...
    def _analyze_call_for_pointers(self, call: Call) -> None:
        ...
        if call.ret_expr is not None:
            ...
            if ret_var is not None:
                if func_name and self._normalize_name(func_name) in POINTER_RETURNING_FUNCTIONS:
    ...
    def _assign_pointer_names(self) -> None:
        ...
        sorted_candidates = sorted(
            self._pointer_candidates.items(),
        ...
        for var, info in sorted_candidates:
            ...
            if info["is_linked_list"]:
                category = "iterator"
            elif info["is_iterator"]:
                category = "iterator"
            else:
                category = "generic"
            ...
            if idx < len(names):
                ...
                new_name = f"ptr{sum(self._name_counter.values())}"

            self._var_to_new_name[var] = new_name
            self._name_counter[category] += 1

            l.debug(
                "Identified pointer %s (score=%d, patterns=%s)",
                var.name, info["score"], info["patterns"]
            )
