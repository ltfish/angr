...
class CallResultNaming(SemanticNamingBase):
    ...
    def _get_function_name(self, call: Call) -> str | None:
        ...
        if isinstance(target, Const):
            # Try to look up the function name from the knowledge base
            # This is a simplified approach
            return None

        # If the target has a name attribute
        if hasattr(target, "name") and target.name:
            return target.name

        # Check if there's a referenced function
    ...
    def _get_name_for_function(self, func_name: str) -> str | None:
