...
BOOLEAN_FLAG_NAMES = ["flag", "found", "done", "ok", "success", "valid", "ready", "exists"]
...
class BooleanNaming(SemanticNamingBase):
    ...
    def _is_comparison_op(self, op: str) -> bool:
        """Check if an operation is a comparison."""
        comparison_ops = {
            "CmpEQ", "CmpNE", "CmpLT", "CmpLE", "CmpGT", "CmpGE",
            "CmpLTS", "CmpLES", "CmpGTS", "CmpGES",  # Signed comparisons
    ...
    def _score_candidates(self) -> None:
        ...
        for var, info in self._bool_candidates.items():
            ...
            if hasattr(var, "size") and var.size is not None:
                if var.size <= 4:  # 4 bytes or less
                    info["score"] += 5
    ...
    def _assign_boolean_names(self) -> None:
        ...
        for var, info in sorted_candidates:
            ...
            l.debug(
                ...
                var.name, info["score"], info["direct_condition"], info["comparison_result"]
            )
